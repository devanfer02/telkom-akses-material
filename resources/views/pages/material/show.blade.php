...
@section('content')
    ...
    <div class="details-container">
        ...
        <div class="details-grid">
            ...
            <div class="detail-item"><strong>Status:</strong> <span
                    class="badge bg-{{ $material->status == 'IN' ? 'success' : 'danger' }}">{{ $material->status }}</span>
            </div>
            <div class="detail-item"><strong>Tanggal:</strong> {{ \Carbon\Carbon::parse($material->date)->format('d M Y') }}
            </div>
        </div>
            ...
            @if ($material->evidence && count(json_decode($material->evidence)) > 0)
                <div class="evidence-grid">
                    @foreach (json_decode($material->evidence) as $evidence)
                        <a href="{{ asset('uploads/' . $evidence) }}" target="_blank">
                            <img src="{{ asset('uploads/' . $evidence) }}" alt="Bukti Foto">
                        </a>
                    @endforeach
                </div>
            @else
        ...
        <a href="{{ route('material.index') }}" class="btn btn-secondary back-button"><i class="fas fa-arrow-left"></i>
            Kembali ke Daftar</a>
    </div>
@endsection
